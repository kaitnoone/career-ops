...
**Every company MUST have a `careers_url` in portals.yml.** If it doesn't, search for it once, save it, and reuse it in future scans.
...
Levels are additive — all run, results get merged and deduplicated.
...
4. **Level 1 — Playwright scan** (parallel in batches of 3-5):
   ...
   g. If `careers_url` fails (404, redirect), try `scan_query` as a fallback and note it for URL update

5. **Level 2 — ATS APIs / feeds** (parallel):
   For each company in `tracked_companies` with `api:` defined and `enabled: true`:
   a. WebFetch the API/feed URL
   b. If `api_provider` is defined, use its parser; otherwise infer from the domain (`boards-api.greenhouse.io`, `jobs.ashbyhq.com`, `api.lever.co`, `*.bamboohr.com`, `*.teamtailor.com`, `*.myworkdayjobs.com`)
...
7.5. **Verify liveness of WebSearch (Level 3) results** — BEFORE adding to pipeline:
   ...
   WebSearch results can be stale (Google caches results for weeks or months). To avoid evaluating expired offers, verify each new URL from Level 3 with Playwright. Levels 1 and 2 are inherently real-time and don't require this check.

   For each new Level 3 URL (sequential — NEVER Playwright in parallel):
   a. `browser_navigate` to the URL
   b. `browser_snapshot` to read the content
   c. Classify:
      - **Active**: role title visible + role description + visible Apply/Submit/Solicitar control within the main content. Do not count generic header/navbar/footer text.
   ...
   **Do not interrupt the whole scan if a URL fails.** If `browser_navigate` errors (timeout, 403, etc.), mark as `skipped_expired` and move on to the next.
...
Every company in `tracked_companies` must have `careers_url` — the direct URL to its jobs page. This avoids having to look it up every time.
