...
3. **WebSearch (last resort):** Search for the role title + company in secondary portals that index the JD as static HTML.
...
2. **Generate answers** following the tone guide (see below).
3. **Save in the report** as section `## H) Draft Application Answers`.

### Generic questions (use if the form can't be parsed)
...
Record in `data/applications.md` with all columns including Report and PDF set to ✅.
